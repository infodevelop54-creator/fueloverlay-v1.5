...
class VisualSettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        content.addView(TextView(this).apply {
            text = "Размер шрифта CWG, sp"
            textSize = 13f
            setTextColor(Color.WHITE)
            setPadding(0, dp(8), 0, dp(4))
        })
        content.addView(makeSlider(appearance.cwgTextSizeSp, 8f, 32f, 1f) {
            update(appearance.copy(cwgTextSizeSp = it))
        })

        content.addView(TextView(this).apply {
            text = "Цвет текста и иконок CWG"
            textSize = 13f
            setTextColor(Color.WHITE)
        ...
        content.addView(CheckBox(this).apply {
            text = "Показывать фон"
            isChecked = appearance.cwgShowBackground
            setTextColor(Color.WHITE)
            setPadding(0, dp(12), 0, dp(4))
            setOnCheckedChangeListener { _, checked ->
                update(appearance.copy(cwgShowBackground = checked))
            }
        })
    ...
    private fun update(a: WidgetAppearance) {
        appearance = a
        AppearanceRepository.save(this, a)
        refreshPreview()
    }
